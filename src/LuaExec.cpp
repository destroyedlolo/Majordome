...
void LuaExec::readConfigDirective( std::string &l ){
	...
	if(!(arg = striKWcmp( l, "-->> need_task=" )).empty()){
		...
		if(d2)
			fd2 << this->getFullId() << " -- " << LuaTask::trigramme() << arg << ": need { style.stroke-dash: 2 }" << std::endl;
		...
		if( (event = config.EventsList.find(arg)) != config.EventsList.end()){
			...
			if(d2)
				fd2 << this->getFullId() << " -- " << event->second->getFullId() << ": need { style.stroke-dash: 2 }" << std::endl;
			return;
		} else {
		...
		if( (topic = config.TopicsList.find(arg)) != config.TopicsList.end()){
			...
			if(d2)
				fd2 << this->getFullId() << " -- " << topic->second->getFullId() << ": need { style.stroke-dash: 2 }" << std::endl;
			return;
		} else {
		...
		if( (topic = config.TopicsList.find(arg)) != config.TopicsList.end()){
			...
			if(d2)
				fd2 << this->getFullId() << " -- " << topic->second->getFullId() << ": require { style.stroke-dash: 2; style.stroke-width: 4; style.stroke: red}" << std::endl;
		...
		if( (timer = config.TimersList.find(arg)) != config.TimersList.end()){
			...
			if(d2)
				fd2 << this->getFullId() << " -- " << timer->second->getFullId() << ": need { style.stroke-dash: 2 }" << std::endl;
			return;
		} else {
		...
		if( (trk = config.TrackersList.find(arg)) != config.TrackersList.end()){
			...
			if(d2)
				fd2 << this->getFullId() << " -- " << trk->second->getFullId() << ": need { style.stroke-dash: 2 }" << std::endl;
			return;
		} else {
			SelLog->Log('F', "\t\ttimer '%s' is not (yet ?) defined", arg.c_str());
			exit(EXIT_FAILURE);
		...
		if( (minmax = config.MinMaxList.find(arg)) != config.MinMaxList.end()){
			...
			if(d2)
				fd2 << this->getFullId() << " -- " << minmax->second->getFullId() << ": need { style.stroke-dash: 2 }" << std::endl;
			return;
		} else {
		...
		if( (nminmax = config.NamedMinMaxList.find(arg)) != config.NamedMinMaxList.end()){
			...
			if(d2)
				fd2 << this->getFullId() << " -- " << nminmax->second->getFullId() << arg << ": need { style.stroke-dash: 2 }" << std::endl;
			return;
		} else {
		...
		if( (shut = config.ShutdownsList.find(arg)) != config.ShutdownsList.end()){
			...
			if(d2)
				fd2 << this->getFullId() << " -- " << shut->second->getFullId() << ": need { style.stroke-dash: 2 }" << std::endl;
			return;
		} else {
...
#	ifdef PGSQL
		...
		if( (shut = config.pgSQLsList.find(arg)) != config.pgSQLsList.end() ){
			...
			if(d2)
				fd2 << this->getFullId() << " -- " << pgSQL::trigramme() << arg << ": need { style.stroke-dash: 2 }" << std::endl;
			return;
		} else {
